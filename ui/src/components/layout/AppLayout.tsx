...
function AppHeader() {
  ...
  // customer-center-platform fork（母 repo W4a D7）：上游頁首右側的「Join Slack」
  // 與 GitHub star 徽章移除（外站社群／行銷連結，交付態無對應；star 計數本身是
  // 一條對外請求）。剩下的只有行動版的選單鈕與品牌字，桌面寬度下整條是空的
  // sticky 列 ⇒ 頁首只在行動寬度渲染（md:hidden）。
  return (
    <header className="sticky top-0 z-50 flex items-center justify-between border-b border-border/60 bg-background/70 px-4 py-2 backdrop-blur-md supports-[backdrop-filter]:bg-background/55 md:hidden">
      <div className="flex items-center gap-3">
        ...
        <Link href="/" className="text-lg font-bold">Dograh</Link>
      </div>
    </header>
  );
}
